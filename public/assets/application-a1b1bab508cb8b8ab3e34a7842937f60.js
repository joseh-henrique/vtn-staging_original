...
(function() {
  var disable_user_information, enable_user_information, requiredFields, resetSelectTxt, setSelectedPlan;

  jQuery(function() {
    var set_as_default_image;
    $(".currency-input").maskMoney();
    $("#supplemental_information").collapse({
      toggle: false
    });
    $(".chzn-select").chosen();
    $(".show_if_checked").hide();
    $('#btn_step_2_wizard_image_upload').click(function() {
      if ($('.img-row').length === 0) {
        alert("Please upload at least one image to continue");
        return false;
      }
    });
    $(".isPair").click(function() {
      if ($(this).is(':checked')) {
        $(".planPrice").hide();
        return $(".planPairPrice").show();
      } else {
        $(".planPrice").show();
        return $(".planPairPrice").hide();
      }
    });
    ...
    if ($('#fileupload').length !== 0) {
      $('#fileupload').fileupload({
        formData: {
          iframe_redirect_to: vtn_constants.BASE_URI + '/result.html?%s',
          authenticity_token: $('meta[name="csrf-token"]').attr('content')
        }
      });
    }
    $('#appraisal_images_table').on('click', '.btn_make_image_primary', (function(event) {
      $.ajax($(this).attr("href"), {
        type: 'POST',
        dataType: 'json',
    ...
    $('#btnSaveComment').click(function() {
      ...
      $.ajax('/appraisals/comment', {
        ...
        dataType: 'json',
        data: {
          comment_appraisal_id: $('#comment_appraisal_id').val(),
          comment: {
        ...
        error: function(data) {
          ...
          $('#div_comment').show();
          return $('#div_saving_comment').hide();
        }
      });
      return false;
    });
    $('#btnSubmitAppraisalReply').click(function() {
    ...
    $("#planprod1").click(function() {
      ...
      return $("#appraisal_selected_plan").val(vtn_constants.EAAppraisalTypeShortRestricted);
    });
    $("#planprod2").click(function() {
      $(".formProd").removeClass("formProdOn");
      ...
      return $("#appraisal_selected_plan").val(vtn_constants.EAAppraisalTypeLongRestricted);
    });
    $("#planprod4").click(function() {
      $(".formProd").removeClass("formProdOn");
      ...
      return $("#appraisal_selected_plan").val(vtn_constants.EAAppraisalTypeLongForSelling);
    });
    $("#plansel1").click(function() {
      $(".formProd").removeClass("formProdOn");
      $("#planprod1").addClass("formProdOn");
      $(".formProdSelect").removeClass("formProdSelected");
      $("#plansel1").addClass("formProdSelected");
    ...
    $('#chkSuggestRejection').click(function() {
      if ($(this).is(':checked')) {
        return $("#divRejectionReason").show();
      } else {
        $("#divRejectionReason").hide();
        return $("#txtRejectionReason").val("");
      }
    });
    $("#chkImportAccount").change(function() {
      if ($(this).is(':checked')) {
        ...
        return $("#appraisal_payment_attributes_zip").val($("#customer_address_attributes_zip").val());
      } else {
        $("#txtCardName").val("");
        $("#appraisal_payment_attributes_address").val("");
        $("#appraisal_payment_attributes_city").val("");
        $("#appraisal_payment_attributes_state").val("");
        return $("#appraisal_payment_attributes_zip").val("");
      }
    });
    $("#chkVtnPartner").change(function() {
      if ($(this).is(":checked")) {
    ...
    $("#btnBuildWizardPhoto").click(function() {
      if ($('.img-row').length === 1) {
        alert("Please upload at least one image to continue");
        return false;
      } else {
    ...
    $("#btnBuildWizardGeneral").click(function() {
      if ($("#appraisal_classification_attributes_category_id").val() === "") {
    ...
    $("#btnBuildWizardPayment").click(function() {
      var form;
      if (requiredFields()) {
        ...
        $.ajax('/payments', {
          ...
          success: function(data) {
            ...
            if (data.status === false) {
              return $("#paymentModalFooter").show();
            } else {
    ...
    $("#paymentModal").on("hide", function() {
      $("#paymentModalLoading").show();
      $("#paymentModalDeclined").hide();
      return $("#paymentModalFooter").hide();
    });
    ...
    return $("#txtAppraisalComments").textareaCount({
      originalStyle: "originalDisplayInfo"
    }, function(data) {
      if (data.words >= 100) {
        return $("#fullSummaryHelp").hide();
      } else {
        return $("#fullSummaryHelp").show();
      }
    });
  });
  ...
  requiredFields = function() {
    ...
    if ($("#chkVtnPartner")[0].checked === true) {
      $("[partner-required]").not(":hidden").each(function() {
        if ($(this).val() === "") {
  ...
  resetSelectTxt = function() {
    $("#plansel1").html("Select");
    $("#plansel2").html("Select");
    $("#plansel4").html("Select");
    $("#small-plansel1").html("Select");
    $("#small-plansel2").html("Select");
    return $("#small-plansel4").html("Select");
  };

  setSelectedPlan = function() {
    if ($("#appraisal_selected_plan").val()) {
      return $("#plansel" + $("#appraisal_selected_plan").val()).trigger('click');
    } else {
      return $("#plansel4").trigger('click');
    }
  };

  disable_user_information = function() {
    $("#chkImportAccount").attr("disabled", "disabled");
    return $("[required]").attr("disabled", "disabled");
  };

  enable_user_information = function() {
    $("#chkImportAccount").removeAttr("disabled");
    return $("[required]").removeAttr("disabled");
  };
...
(function() {
  jQuery(function() {
    ...
    $('form').on('click', '.add_fields', function(event) {
      ...
      event.preventDefault();
      return $(".phone-input").mask("(999)-999-9999");
    });
    $(".phone-input").mask("(999)-999-9999");
    $("#btn-appraiser_steps-add_references").click(function(event) {
      if ($(".trade-references-inputs:visible").length < 3) {
        alert("You must add at least 3 references");
        return event.preventDefault();
      }
    });
    $("form[action*='/appraiser_steps/preferences']").submit(function(event) {
      if (($('#customer_notify_by_sms').is(':checked') || $('#customer_notify_by_email').is(':checked')) || ($('#appraiser_notify_by_sms').is(':checked') || $('#appraiser_notify_by_email').is(':checked'))) {
        if (!($('#accept_reimburse').is(':checked'))) {
          alert('Please accept to not be reimbursed to continue');
          return event.preventDefault();
        }
      }
    });
    $('#btn_save_bank_step').click(function(event) {
      if (!($("#appraiser_paypal_email").val())) {
        alert("Please enter an email address associated with your Paypal account to continue");
        return false;
      }
    });
    return $('#btn_submit_signature').click(function(event) {
...
(function() {
  jQuery(function() {
    ...
    $("ul.nav li.dropdown").hover((function() {
      return $(this).find(".dropdown-menu").stop(true, true).delay(200).fadeIn();
    }), function() {
      return $(this).find(".dropdown-menu").stop(true, true).delay(200).fadeOut();
    });
    ...
    return $('#new_appraiser').submit(function() {
...
(function() {
  jQuery(function() {
    var as_currency, calculate_discount, clear_coupon_badge, set_coupon_badge, toggle_coupon_divs, update_totals, validate_coupon;
    toggle_coupon_divs = function(element) {
      $("#valid_coupon").hide();
      $("#invalid_coupon").hide();
      $("#processing_coupon").hide();
      return $(element).show();
    };
    set_coupon_badge = function(status) {
      var css_icon, css_style, text;
      switch (status) {
        case 'error':
          css_style = 'danger';
          css_icon = 'remove';
      ...
      return $("#payment-coupon-div .help-block").html('<span class="label label-' + css_style + '"><span class="glyphicon glyphicon-' + css_icon + '"></span> ' + text + '</span>');
    };
    clear_coupon_badge = function(element) {
      return $("#payment-coupon-div .help-block").html('');
    };
    calculate_discount = function(price, discount, type) {
      var ret;
      if (discount === 0) {
        ret = [price, 0, price];
      } else {
    ...
    as_currency = function(value) {
      return "$" + parseFloat(value, 10).toFixed(2);
    };
    update_totals = function(amounts) {
      $("#td_appraisal_cost").text(as_currency(amounts[0]));
      $("#td_appraisal_discount").text(as_currency(amounts[1]));
      return $("#td_appraisal_total").text(as_currency(amounts[2]));
    };
    validate_coupon = function(params) {
      return $.ajax("/validate_coupon", {
        type: 'GET',
        dataType: 'json',
        data: {
        ...
        success: function(data) {
          ...
          return update_totals(calculate_discount($("#appraisal_price").val(), data.discount, data.discount_type));
        },
        error: function(data) {
          set_coupon_badge('error');
          return update_totals(calculate_discount($("#appraisal_price").val(), 0, "fixed"));
        }
      });
    };
    $("#appraisal_payment_attributes_coupon").keyup(function() {
    ...
    return $("#processXW").change(function() {
      if ($("#appraisal_payment_attributes_coupon").val().length !== 16) {
        clear_coupon_badge();
        update_totals(calculate_discount($("#appraisal_price").val(), 0, "fixed"));
        return false;
...
(function() {
  jQuery(function() {
    $(".btn_make_image_primary").click(function() {
      if ($(this).hasClass('btn-success')) {
        return $(this).removeClass('btn-sucess disabled');
      } else {
        $(".btn_make_image_primary").removeClass("btn-success disabled");
        $(".btn_make_image_primary").html('<i class="icon-picture"></i> Mark as Primary');
        $(this).addClass('btn-success disabled');
        return $(this).html('<i class="icon-picture icon-white"></i> Primary');
      }
    });
    return $(".btn_delete_image").click(function() {
...
(function() {
  jQuery(function() {
    return $("#btn_export_1099").click(function(event) {
      var a_href, key;
      a_href = $(this).attr('href');
      key = $("#txt_tax_code").val();
      return $(this).attr('href', a_href + "?code=" + key);
    });
  });
